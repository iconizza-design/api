 ...
 * Storage
 */
export const iconSetsStorage = createStorage<IconizzaIcons>(storageConfig);
...
export function storeLoadedIconSet(
	...
	storage: MemoryStorage<IconizzaIcons> = iconSetsStorage,
	...
	// Stored items
	const splitItems: SplitRecord<MemoryStorageItem<IconizzaIcons>>[] = [];
	const storedItems: MemoryStorageItem<IconizzaIcons>[] = [];
	...
	splitRecords(
		...
		(splitIcons, next, index) => {
			// Store data
			createStoredItem<IconizzaIcons>(storage, splitIcons.data, cachePrefix + index, true, (storedItem) => {
