...
ARG ICONIZZA_API_VERSION=3.0.0
ARG SRC_PATH=./
...
# Set work directory
WORKDIR /data/iconizza-api

#### Stage iconizza-api-install #########################################################################################
FROM base AS iconizza-api-install
ARG SRC_PATH
...
# Copy src and icons
COPY ${SRC_PATH}src/ /data/iconizza-api/src/
COPY ${SRC_PATH}icons/ /data/iconizza-api/icons/

# Build API
RUN npm run build

#### Stage RELEASE #####################################################################################################
FROM iconizza-api-install AS RELEASE
ARG BUILD_DATE
ARG BUILD_VERSION
ARG BUILD_REF
ARG ICONIZZA_API_VERSION
ARG ARCH
...
LABEL org.label-schema.build-date=${BUILD_DATE} \
    ...
    org.label-schema.name="Iconizza API" \
    org.label-schema.version=${BUILD_VERSION} \
    org.label-schema.description="Node.js version of api.iconizza.design" \
    org.label-schema.url="https://github.com/iconizza/api" \
    org.label-schema.vcs-ref=${BUILD_REF} \
    org.label-schema.vcs-type="Git" \
    org.label-schema.vcs-url="https://github.com/iconizza/api" \
    org.label-schema.arch=${ARCH} \
    authors="Dennis Ollhoff"

RUN rm -rf /tmp/*

# Env variables
ENV ICONIZZA_API_VERSION=$ICONIZZA_API_VERSION

# Expose the listening port of Iconizza API
EXPOSE 3000
