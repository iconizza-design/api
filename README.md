...
This repository contains Iconizza API script. It is a HTTP server, written in Node.js that:
...
To run a Docker image, run `docker run -d -p 3000:3000 iconizza/api` (change first 3000 to port you want to run API on).
...
-   Automatically load latest icons from [`@iconizza/json`](https://github.com/iconizza/icon-sets).
...
-   `ICONIZZA_SOURCE=full`: source for Iconizza icon sets. Set to `full` to use `@iconizza/json` package, `split` to use `@iconizza-json/*` packages, `none` to use only custom icon sets.
-   `REDIRECT_INDEX=https://iconizza.design/`: redirect for `/` route. API does not serve any pages, so index page redirects to main website.
-   `STATUS_REGION=`: custom text to add to `/version` route response. Iconizza API is ran on network of servers, visitor is routed to closest server. It is used to tell which server user is connected to.
...
See [memory management in full API docs](https://docs.iconizza.design/api/hosting-js/config.html).
...
Full documentation is available on [Iconizza documentation website](https://docs.iconizza.design/api/).
...
Iconizza API is licensed under MIT license.
...
© 2022 Dennis Ollhoff / Iconizza OÜ
