import type { ExtendedIconizzaIcon, IconizzaIcons, IconizzaJSON } from '@iconizza/types';
...
describe('Loading icon data from storage', () => {
	test('Testing mdi', async () => {
		const iconSet = JSON.parse(await loadFixture('json/mdi.json')) as IconizzaJSON;
		...
		function store(): Promise<StoredIconSet> {
			return new Promise((fulfill, reject) => {
				...
				const storage = createStorage<IconizzaIcons>({
					cacheDir,
					maxCount: 2,
				});
		...
		function getIcon(name: string): Promise<ExtendedIconizzaIcon | null> {
			return new Promise((fulfill, reject) => {
	...
	test('Synchronous loading', async () => {
		const iconSet = JSON.parse(await loadFixture('json/mdi-light.json')) as IconizzaJSON;
